...
# Размер страницы в символах
PAGE_SIZE = int(os.getenv('PAGE_SIZE', 2000))

# Максимальное количество книг на пользователя
...
USERS_JSON_FILE = os.getenv('USERS_JSON_FILE', 'users.json')
PENDING_USERS_JSON_FILE = os.getenv('PENDING_USERS_JSON_FILE', 'pending_users.json')
LOG_FILE = os.getenv('LOG_FILE', '/app/logs/bot.log')
DB_FILE = os.getenv('DB_FILE', 'reader_data.db')
...
def get_book_file(book_info):
    ...
    archive_path = os.path.join(BOOKS_DIR, archive_name)
...
# захват ссылок с флибуста
@bot.message_handler(func=lambda message: is_user_approved(message.from_user.id) and re.search(r'flibusta\.is/b/(\d+)', message.text))
def handle_flibusta_link_message(message):
    chat_id = message.chat.id
    ...
    try:
        link = message.text
        match = re.search(r'flibusta\.is/b/(\d+)', link)

        if not match:
            # This should ideally not happen due to the handler's func, but as a fallback
            bot.send_message(chat_id, "Неверный формат ссылки. Пожалуйста, введите ссылку в формате `http://flibusta.is/b/XXXXXX`.")
        ...
        if file_path and os.path.exists(file_path):
            try:
                with open(file_path, 'rb') as book_file:
                    ...
                    bot.send_document(chat_id, book_file, caption=f"{full_filename}\nСсылка на сайт: [link](http://flibusta.is/b/{libid})", parse_mode="Markdown")
...
def handle_info_button(message):
    ...
    response = (
        "Привет! \nЯ бот для поиска и скачивания книг из библиотеки Flibusta.\n\n"
        "Библиотека [Flibusta](http://flibusta.is) - это бесплатная библиотека, некогда - зеркало Либрусек, после монетизации последней ушедшая в свободное плавание. В раздаче присутствуют книги как до разделения библиотек, так и появившиеся на Флибусте после.\n\n"
        f"Используемая база книг: [База Flibusta](https://booktracker.org/viewtopic.php?t=46979)\n"
        ...
        f"Если не нашел свою книгу во Flibusta, можно поискать ее на [LibRusEc](https://t.me/LibRusEcBase_bot).\n\n"
        ...
        try:
            with open(file_path, 'rb') as book_file:
                ...
                bot.send_document(
                    chat_id,
                    book_file,
                    caption=f"{full_filename}\nСсылка на сайт: [link](http://flibusta.is/b/{link_book})",
                    parse_mode="Markdown"
                )
...
def display_results(chat_id):
    ...
    for i, book in enumerate(books_to_display):
        ...
        book_id = book['LIBID']
        book_link = f"http://flibusta.is/b/{book_id}"
